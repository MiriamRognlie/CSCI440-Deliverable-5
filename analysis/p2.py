...
data = pandas.read_sql_query(
    "SELECT Runtime, (cast(GrossProfit AS SIGNED) - cast(Budget AS SIGNED )) AS Profit FROM movie WHERE Country = 'USA' AND GrossProfit IS NOT NULL AND Budget IS NOT NULL and Runtime is not null",
    conn)
# Run a query so "data2" is a table containing the runtime of all movies and their ratings
data2 = pandas.read_sql_query("SELECT Runtime, Rating FROM movie WHERE Rating IS NOT NULL and Country = 'USA' and Runtime is not null", conn)
# Lets create
