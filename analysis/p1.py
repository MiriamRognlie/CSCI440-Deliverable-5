...
def p1(db):
    ...
    # Gross Profit as actually just revenue. Run a query so "data" is a table containing the Budget for a movie and the
    # profit returned from the box office.  This displays only movies from the USA to ensure a common currency
    data = db.query("SELECT Budget, (cast(GrossProfit AS SIGNED) - cast(Budget AS SIGNED )) AS Profit FROM movie WHERE Country='USA' AND Budget IS NOT NULL AND GrossProfit IS NOT NULL")
