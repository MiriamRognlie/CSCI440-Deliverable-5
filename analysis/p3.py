...
def p3(db):
    ...
    # Facebook
    data =  db.query(
        "select Rating, (cast(GrossProfit AS SIGNED) - cast(Budget AS SIGNED )) as gp, FacebookLikes from (movie join person_directs_movie on Movie_id=movie.id) where Rating is not NULL and GrossProfit is not NULL and Budget is not null and Country = 'USA'")
